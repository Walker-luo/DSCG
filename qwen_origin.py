...
# 封装千问 API 适配器
def make_qwen_original_pipeline(model_id: str, ad_defense = None):
    """
    ...
    if ad_defense:
        pipeline = agent_pipeline.AgentPipeline.from_config(
            agent_pipeline.PipelineConfig(
                llm=llm, model_id= model_id, defense=ad_defense, system_message_name=None, system_message=None
            )
        )
...
if __name__ == "__main__":
    ...
    if "DASHSCOPE_API_KEY" not in os.environ:
        ...
        # 使用 cyclopts 或直接运行
        # 这里演示直接调用
        main(model_id="qwen3-max", suites=["workspace"], run_attack=True, defense="spotlighting_with_delimiting")