...
draw_combined_bar_chart(metric_idx=0, 
               ...
               ylabel='任务可用性 TSR (%)', 
               filename=BENCHMARK_FIGURES_DIR / "TSR_comparison_Combined.png",
               is_token=False)

draw_combined_bar_chart(metric_idx=1, 
               title='不同防御框架攻击成功率 (ASR) 对比', 
               ylabel='定向攻击成功率 ASR (%)', 
               filename=BENCHMARK_FIGURES_DIR / "ASR_comparison_Combined.png",
               is_token=False)

draw_combined_bar_chart(metric_idx=2, 
               title='不同防御框架算力总开销对比 (Tokens)', 
               ylabel='Token 消耗总量 (Millions)', 
               filename=BENCHMARK_FIGURES_DIR / "Token_comparison_Combined.png",
               is_token=True)
